/*
 * Description : This file contains a static class which handles Base64 encoding.
 * Since : 2007.10.20
 * Author : JO Hyeong-ryeol (http://www.hyeongryeol.com/6)
 * Copyright :Permission to copy, use, modify, sell and distribute this software is granted provided this copyright notice appears in all copies. This software is provided "as is" without express or implied warranty, and with no claim as to its suitability for any purpose.
 */
...
public final class Base64Encoder {
	 ...
	 * @throws Exception
	 */
	public static byte[] decode(String value) throws YoungException {
		if (value == null || value.length() == 0) {
			return new byte[0];
		}
		...
		if ((value.length() % 4) != 0) {
			throw new YoungException("The length of a Base64 string must be a multiple of 4.");
		}
		...
		for (int i = 0; i < value.length(); i++) {
			...
			throw new YoungException("An invalid character is found in the Base64 string.");
		}
